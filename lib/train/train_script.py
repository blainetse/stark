...
def run(settings):
    settings.description = (
        "Training script for STARK-S, STARK-ST stage1, and STARK-ST stage2"
    )
    ...
    # TODO: 4.Loss functions and Actors
    if settings.script_name == "stark_s" or settings.script_name == "stark_st1":
        ...
        actor = STARKSActor(
            net=net, objective=objective, loss_weight=loss_weight, settings=settings
        )
    ...
    # TODO: 5.Optimizer, parameters, and learning rates
    ...
    if settings.script_name in ["stark_st2", "stark_st2_plus_sp"]:
        trainer.train(
            cfg.TRAIN.EPOCH, load_latest=True, fail_safe=True, load_previous_ckpt=True
        )
    else:
